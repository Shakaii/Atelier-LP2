...
db.once('open', function() {
	...
	app.get("/validate/:id",function (req,res){
		...
		User.findOne({email:req.session.email}, function (err, user){
			...
			let categ="";
			box=user.boxes.id(req.params.id);
				
			box.prestations.forEach(function(prest){
			
				Category.find({},function(err,doc){
	
					doc.forEach(function(categorie){
				
						if(categorie.prestations.id(prest._id)){
							
							if(categorie._id != categ){
			...
			res.render('validate',{'mail':req.session.email,'categorie':cate});
